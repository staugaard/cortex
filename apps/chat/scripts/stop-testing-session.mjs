...
const PROCESS_PATTERNS = [
	...
	"vite --port 5174",
	"electrobun dev",
	"/apps/chat/build/dev-macos-arm64/react-tailwind-vite-dev.app/Contents/MacOS/bun",
];
...
async function main() {
	console.log("[chat stop] Closing Playwright CLI sessions...");
	run("playwright-cli", ["close-all"]);
	run("playwright-cli", ["kill-all"]);

	console.log("[chat stop] Sending SIGINT to chat dev/test processes...");
	...
	if (remaining.length > 0) {
		console.log(
			`[chat stop] Escalating to SIGTERM for remaining pids: ${remaining.join(", ")}`,
		);
		for (const pattern of PROCESS_PATTERNS) {
	...
	if (remaining.length > 0) {
		console.log(
			`[chat stop] Escalating to SIGKILL for remaining pids: ${remaining.join(", ")}`,
		);
		for (const pattern of PROCESS_PATTERNS) {
	...
	if (portBound.length > 0) {
		console.log(
			`[chat stop] Releasing bound ports (${PORTS.join(", ")}) from pids: ${portBound.join(", ")}`,
		);
	...
	if (remaining.length > 0) {
		console.log(
			`[chat stop] Some matching processes are still running: ${remaining.join(", ")}`,
		);
		process.exitCode = 1;
		return;
	}
	if (portBound.length > 0) {
		console.log(
			`[chat stop] Some port-bound processes are still running: ${portBound.join(", ")}`,
		);
		process.exitCode = 1;
		return;
	}

	console.log("[chat stop] Done. No matching chat dev/test processes remain.");
}
